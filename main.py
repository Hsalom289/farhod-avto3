# -*- coding: utf-8 -*-
"""
Forward bot: (eski + yangi) postlarni aylantiradi.
- Avval manba kanalda post/captionni tahrirlaydi:
  * keraksiz t.me/... va eski instagram linklarini olib tashlaydi
  * pastiga TELEGRAM/INSTAGRAM so'zlarini YASHIRIN hyperlink sifatida qo'shadi
- Tahrirlashga huquq bo'lmasa: automatik o'chadi, copy (HTML) bilan yuboradi
- Forward taqiqlansa: manual copy (matn/media/album) fallback
...
# Exclude (hech qachon yuborilmaydi) — manbani ham qo'shing
EXCLUDED_TARGETS = [
...
# Yashirin hyperlinklar uchun rasmiy havolalar
...
# Kanaldagi tahrirlashni avtomatik boshqarish
ENABLE_SOURCE_EDIT = True
_can_edit_source_cache = None
...
def remove_unwanted_links(text: str) -> str:
    if not text:
        return ""
    text = BLOCK_LINES_PAT.sub('', text)           # eski TELEGRAM/INSTAGRAM satrlarini olib tashlash
    text = INSTAGRAM_PAT.sub('', text)             # eski instagram profil linklarini olib tashlash
    text = TME_ANY_PAT.sub('', text)               # barcha t.me linklarni olib tashlash
    text = re.sub(r'[ \t]+\n', '\n', text)         # trailing space
...
def append_signature_html(text: str) -> str:
    ...
    base_html = html.escape(base)  # foydalanuvchi matnini HTML-safe qilish
    block = make_signature_html()

    # Agar allaqachon TELEGRAM/INSTAGRAM so'zlari blok ko'rinishida bo'lsa, tozalab qayta qo'shamiz
    base_html = BLOCK_LINES_PAT.sub('', base_html).strip()
    new_text = (base_html + block).strip()

    if len(new_text) > 4096:       # Telegram limit
...
async def try_edit_message_in_source(client: TelegramClient, source_ent, msg) -> bool:
    """
    Kanaldagi xabar/captionni joyida tahrirlaydi (HTML hyperlinked).
    Edit huquqi yo'q yoki xabar turi tahrirlab bo'lmasa -> False.
    """
    try:
        # service yoki poll bo'lsa tahrirlamaymiz
        ...
        logging.warning(f"Edit failed msg_id={getattr(msg,'id','?')}: {e}")
        return False
...
async def main():
    ...
    try:
        ...
        while True:
            ...
            # 1b) Avval KANAL UZIDA tahrir (agar huquq bo'lsa) — avto-detekt
            if ENABLE_SOURCE_EDIT:
                can_edit = _can_edit_source_cache
                if can_edit is None:
                    probe = None
                    for grp in reversed(source_groups):
                        for mm in reversed(grp):
                            txt = (mm.text or (mm.message if hasattr(mm, 'message') else "") or "")
                            if txt and not getattr(mm, 'poll', None) and getattr(mm, 'action', None) is None:
                                probe = mm
                                break
                        if probe:
                            break
                    if probe:
                        ok = await try_edit_message_in_source(client, source_ent, probe)
                        _can_edit_source_cache = ok
                        can_edit = ok
                        await asyncio.sleep(0.5)  # anti-flood
                    else:
                        _can_edit_source_cache = False
                        can_edit = False
                ...
                if can_edit:
                    ...
                    logging.info("Source edit disabled (no rights or unsupported). Using copy-only.")
            else:
                logging.info("Source edit is turned OFF by settings.")
